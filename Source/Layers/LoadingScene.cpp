...
bool LoadingScene::init()
{
    ...
    progressLabel = Label::createWithBMFont("chatFont.fnt", "0%");

    if (progressLabel)
    {
        progressLabel->setPosition({ center.x, center.y - 100 });
        addChild(progressLabel);
        printf("reference count: %d\n", progressLabel->getReferenceCount());
    }
    else printf("nullptr\n");
